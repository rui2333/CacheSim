...
class CacheArray : public GlobAlloc {
    public:
        ...
        virtual int32_t lookupL2(const Address lineAddr, const MemReq* req, bool updateReplacement) = 0;

        /* Runs replacement scheme, returns tag ID of new pos and address of line to write back*/
        virtual uint32_t preinsertL2(const Address lineAddr, const MemReq* req, Address* wbLineAddr) = 0;
         ...
         * allowed to keep internal state in preinsert() and use it in postinsert()
         */
        virtual void postinsertL2(const Address lineAddr, const MemReq* req, uint32_t lineId) = 0;
...
class SetAssocArray : public CacheArray {
    ...
    public:
        SetAssocArray(uint32_t _numLines, uint32_t _assoc, ReplPolicy* _rp, HashFamily* _hf, const char * _cname);
        ...
        int32_t lookupL2(const Address lineAddr, const MemReq* req, bool updateReplacement);
        uint32_t preinsertL2(const Address lineAddr, const MemReq* req, Address* wbLineAddr);
        void postinsertL2(const Address lineAddr, const MemReq* req, uint32_t candidate);


        Line* refresh (const Address lineAddr, const MemReq* req, uint32_t candidate);     
};

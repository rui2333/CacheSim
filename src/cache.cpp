...
uint64_t Cache::access(MemReq& req) {
    //memAccess ++;
    bool* downgrade = (bool*) malloc(sizeof(bool));
    uint64_t respCycle = req.cycle;
    bool skipAccess = cc->startAccess(req); //may need to skip access due to races (NOTE: may change req.type!)
    if (likely(!skipAccess)) {
        bool updateReplacement = (req.type == GETS) || (req.type == GETX);
        int32_t lineId;
        if(strcmp(name.c_str(),"l2-0")==0)
            lineId = array->lookupL2(req.lineAddr, &req, updateReplacement);
        else
            lineId = array->lookup(req.lineAddr, &req, updateReplacement);
        respCycle += accLat;
        
        // record the number of request types
        if(req.type == PUTX)
          DirtyWB ++;
        ...
        if (lineId == -1 && cc->shouldAllocate(req)) {
            ...
            Address wbLineAddr;
            if(strcmp(name.c_str(),"l2-0")==0)
                lineId = array->preinsertL2(req.lineAddr, &req, &wbLineAddr); //find the lineId to replace
            else
            ...
            cc->processEviction(req, wbLineAddr, lineId, respCycle, downgrade); //1. if needed, send invalidates/downgrades to lower level
            
            if(strcmp(name.c_str(),"l2-0")==0)
                array->postinsertL2(req.lineAddr, &req, lineId); //do the actual insertion. NOTE: Now we must split insert into a 2-phase thing because cc unlocks us.
            else
